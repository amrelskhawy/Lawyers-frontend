...
/**
 * Quill 2's getSemanticHTML() (what p-editor emits) escapes every space as &nbsp;.
 * That turns each paragraph into a single unbreakable word, so it refuses to wrap at
 * spaces — markers like "أ-" get stranded on their own line and words split mid-word.
 */
...
export class LawyerFeesContractPreview {
  ...
  page2Html = computed<SafeHtml>(() => {
    ...
    const html = isEffectivelyEmptyHtml(v) ? PAGE2_DEFAULT : normalizeSpaces(v as string);
    return this.sanitizer.bypassSecurityTrustHtml(html);
  });

  page3Html = computed<SafeHtml>(() => {
    const v = this.data().page3Content;
    const html = isEffectivelyEmptyHtml(v) ? PAGE3_DEFAULT : normalizeSpaces(v as string);
    return this.sanitizer.bypassSecurityTrustHtml(html);
  });
